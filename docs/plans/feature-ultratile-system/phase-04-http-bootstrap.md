...
- Requirements for this phase:
  - **REQ-003**: Strict HTTP/1.1 subset: GET-only (`405` + `Allow: GET`), origin-form + absolute-form normalization, exactly-one valid Host, headers as `Map<String,List<String>>` (multiplicity preserved; duplicates detected pre-collapse), `writeFully`/`readFully`, leftover bytes only after bodyless valid upgrade; UTP/1.0 over RFC 6455 documented.
...
- Prior-phase deps:
  - **DEP-003**: Requires phases 01 (pins, `Config`, stub, `build.sh`) + 02 (`ImageRegistry` fresh-snapshot, `.ready` demos, strict meta).
...
| ---- | ------------------------------------------------- | ---------- | --------- | --------- | ---- |
| TASK-001 | Implement `NioHttpServer.handle` strict subset in `src/main/java/com/ultratile/net/NioHttpServer.java` (extend phase-01 stub): `readFully` to `\r\n\r\n` (16 KB→431); parse headers into `LinkedHashMap<String,List<String>>` with lowercase names — multiplicity PRESERVED (a `Map<String,String>` would silently overwrite the first `Host`/`Content-Length` and defeat duplicate detection — v1.6 bug); exactly one syntactically valid `Host` value else 400 (covers missing/duplicate/invalid); origin-form + absolute-form (normalize→path+query, reject others 400); byte-based `Content-Length` (zero values→must agree, conflicting/multiple→400); method gate FIRST (`!=GET` → `405` + `Allow: GET` + close); `writeFully` loop; exact routes `/`,`/viewer.js`,`/styles.css`,`/api/images`,`/api/images/{id}/info`,`/healthz`,`/ws`; `Connection: close` except upgraded `/ws`. For `/ws`: reject TE/any-`CL!=0`/conflicts (400, no upgrade); preserve post-header bytes ONLY after bodyless valid upgrade (else discard with channel). | — | `mvn -q compile` passes |  |  |
...
| TASK-004 | Self-contained validation on SPLIT LINES with LOUD readiness: build with Maven; own line `java -jar ... &` + `pid=$!` + `trap 'kill "$pid"' EXIT`; `ready=0` loop (≤40×2s) on `/healthz` then `[ "$ready" = "1" ] || { echo ...; kill "$pid"; exit 1; }`; `curl` set (static/info/404 + POST→405 with `Allow: GET` + duplicate-`Host`→400 probe via raw socket); absolute-form probe expecting `HTTP/1.1 200`; `rg https?:// web/` empty. Never `kill %1`, never fixed `sleep`, never silent fall-through. | TASK-002 | full block green |  |  |
...
- Header multiplicity is a security property: duplicate-`Host` and conflicting-`Content-Length` detection is IMPOSSIBLE after collapsing to single values — parse to lists first, validate, then select.
- Every retry loop in every phase carries an explicit loud-failure tail; silent fall-through to cleanup is forbidden.
