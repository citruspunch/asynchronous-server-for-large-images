...
| TASK-004 | Create `NEW src/main/java/com/ultratile/proto/UTP_SPEC.md` (18 lines): offset tables 28/8/8/24/16, DataView notes (`getUint32` — JS is unsigned-safe; Java MUST use `toUnsignedLong`), seal incl. empty-gen END 0/0 + END-only-non-canceled-active rule, freeze/mismatch-invalid, validate-before-supersede + seen-advancement table (first-valid-chunk: advance; same-chunk/COMMIT/ABORT/reject: never), ABORT `(imageId,reqId)` match rule, GEN_TILE_CAP pre-insert reject, no-wrap + no-reset-on-switch, FORMAT map. | TASK-002 | `grep -q lastReqIdSeen UTP_SPEC.md` |  |  |
...
- Seen-advancement table is the frozen anti-poisoning rule; ABORT's exclusion is load-bearing (a future ABORT must not shift history).
